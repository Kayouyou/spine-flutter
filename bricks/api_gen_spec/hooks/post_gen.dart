...
void run(HookContext context) {
  final specPath = context.vars['spec'] as String;
  ...
  if (!File(specPath).existsSync()) {
    ...
    resolvedPath = '$projectRoot/$specPath';
  } else {
    resolvedPath = specPath;
  }

  if (!File(resolvedPath).existsSync()) {
    context.logger.err('ERROR: Spec file not found: $resolvedPath');
    exit(1);
  }
  ...
  final projectRoot = _findProjectRoot();
  final apiDir = '$projectRoot/packages/infrastructure/api';
  ...
  for (final entry in models.entries) {
    ...
    modelNames.add(name);
    _writeModel(modelsDir, name, def, context.logger);
    if (def['hive'] == true) {
      _writeHiveModel(modelsDir, name, def, context.logger);
    }
  }

  // Step 2: Generate Retrofit API
  _writeApi(apiFileDir, domain, spec, models, endpoints, context.logger);

  // Step 3: Update barrel
  _updateBarrel(apiDir, modelNames, domain, context.logger);

  context.logger.success('✅ Generated ${modelNames.length} DTOs + ${domain}Api for $domain');
}
...
String _findProjectRoot() {
  ...
  for (final d in candidates) {
    if (Directory('$d/packages/infrastructure/api').existsSync()) {
      return d;
    }
  }
  throw Exception('Project root not found (packages/infrastructure/api missing)');
}

void _writeModel(String dir, String name, Map<String, dynamic> def, Logger logger) {
  ...
  logger.info('  📝 $snake.dart');
}

void _writeHiveModel(String dir, String name, Map<String, dynamic> def, Logger logger) {
  final snake = _toSnake(name);
  ...
  logger.info('  📦 $snake.cm.dart');
}

void _writeApi(String dir, String domain, Map<String, dynamic> spec, Map<String, dynamic> models, List<Map<String, dynamic>> endpoints, Logger logger) {
  final buffer = StringBuffer();
  ...
  logger.info('  🔌 ${domain}_api.dart');
}

void _updateBarrel(String apiDir, List<String> modelNames, String domain, Logger logger) {
  ...
  if (newExports.isNotEmpty) {
    ...
    logger.success('✅ Barrel: +${newExports.length} exports');
  }
}
